...
function renderHtml({ title, description, date, readTime, tags, content, accentGradient, previousWriteup, nextWriteup, featuredUrl, event, difficulty, points }) {
    ...
    <div class="container mx-auto max-w-7xl px-4 sm:px-6 lg:px-4 py-8 relative">
        <header class="mb-12">
            ...
            <div class="flex items-center gap-4 text-sm text-gray-400 mb-6">
                ...
                ${event ? `<span>•</span><span class="text-blue-300">${escapeAttr(event)}</span>` : ''}
                ${difficulty ? `<span>•</span><span class="text-yellow-300">${escapeAttr(difficulty)}</span>` : ''}
                ${points ? `<span>•</span><span class="text-green-300">${points} pts</span>` : ''}
                <span>•</span>
...
function buildAll() {
  const writeupDir = join(__dirname, '../content/writeups');
  const markdownFiles = collectMarkdownFiles(writeupDir);
